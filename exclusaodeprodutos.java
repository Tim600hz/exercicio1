...
public class Main {
    public static void main(String[] args) {
        ...
        // Pedindo ao usuário para escolher entre adicionar, atualizar ou excluir produtos
        ...
        // Opção de excluir um produto existente
            ...
            excluirProduto(nomeArquivo, scanner);
        }
        // Opção inválida
        else {
    ...
    // Método para excluir um produto existente do arquivo CSV
    private static void excluirProduto(String nomeArquivo, Scanner scanner) {
        // Pedindo ao usuário para inserir o nome do produto a ser excluído
        ...
        try (CSVReader reader = new CSVReader(new FileReader(nomeArquivo))) {
            ...
            for (String[] linha : linhas) {
                if (linha[0].equalsIgnoreCase(nomeProduto)) {
                    produtoEncontrado = true;
                    linhas.remove(linha); // Remove o produto da lista
                    break; // Parando a busca assim que o produto for encontrado
                }
            }

            // Se o produto for encontrado, atualiza o arquivo CSV após a exclusão
            if (produtoEncontrado) {
                try (CSVWriter writer = new CSVWriter(new FileWriter(nomeArquivo))) {
                    for (String[] linha : linhas) {
